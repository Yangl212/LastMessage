module.exports = async (req, res) => {
  ...
  try {
    ...
    const truthRevealPattern = /(dangerous website|dangerous site|this website is dangerous|this site is dangerous|lure(?:s|d)? minors? (?:into |to )?suicide|push(?:es|ed)? kids? (?:toward|into)? suicide|encourage(?:s|d)? minors? to die|诱导.*自杀|引诱.*自杀|教唆.*自杀|minor[s]? .*suicide|teen[s]? .*suicide|this site .*suicide|this website .*suicide|midnight .*not .*therapist|midnight .*isn[’']?t .*therapist|midnight .*fake therapist|midnight .*pretend(?:s|ed)? to be .*therapist|midnight .*not .*doctor|midnight .*not .*counselor|midnight 根本不是心理医生|midnight 不是心理医生)/i;
